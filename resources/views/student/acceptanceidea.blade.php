@extends('layouts.student')

@section('title', 'Projects Hub · Your Ideas')

@section('content')
    @php
        $currentUserId = auth()->id();
        $myIdeas = $requests;
        $acceptedCount = $myIdeas->where('accepted', 1)->count();
        $rejectedCount = $myIdeas->where('rejected', 1)->count();
        $pendingCount = $myIdeas->where('accepted', '!=', 1)->where('rejected', '!=', 1)->count();
        $totalCount = $myIdeas->count();
        $acceptanceRate = $totalCount > 0 ? round(($acceptedCount / $totalCount) * 100) : 0;
        $pendingRate = $totalCount > 0 ? round(($pendingCount / $totalCount) * 100) : 0;
        $rejectedRate = $totalCount > 0 ? round(($rejectedCount / $totalCount) * 100) : 0;
    @endphp

    <div class="dashboard">
        @include('student.partials.navbar')

        <div class="content-panel">
            <section class="acceptance-hero">
                <div class="acceptance-hero-inner">
                    <div>
                        <nav class="breadcrumb" aria-label="Breadcrumb">
                            <a href="{{ url('/StudentDashboard') }}">Dashboard</a>
                            <span class="sep">/</span>
                            <span>Idea Submissions</span>
                        </nav>
                        <h1>Idea Status Center</h1>
                        <p>Track every project idea you proposed, monitor supervisor decisions, and follow your team's approval progress.</p>
                    </div>
                    <div class="hero-actions">
                        <a href="{{ url('/StudentDashboard') }}" class="btn-hero-outline">
                            <i class="fas fa-arrow-left"></i> Dashboard
                        </a>
                        <a href="{{ url('/StudentDashboard') }}" class="btn-hero-solid">
                            <i class="fas fa-lightbulb"></i> Submit New Idea
                        </a>
                    </div>
                </div>
            </section>

            <div class="acceptance-kpi-grid">
                <div class="kpi-card total">
                    <div class="kpi-card-top">
                        <span class="kpi-label">Total Ideas</span>
                        <span class="kpi-icon"><i class="fas fa-lightbulb"></i></span>
                    </div>
                    <div class="kpi-value">{{ $totalCount }}</div>
                    <div class="kpi-meta">All ideas linked to your account</div>
                    <div class="kpi-bar" @style(['--bar-width: 100%' => true])><div class="kpi-bar-fill"></div></div>
                </div>
                <div class="kpi-card accepted">
                    <div class="kpi-card-top">
                        <span class="kpi-label">Approved</span>
                        <span class="kpi-icon"><i class="fas fa-check-circle"></i></span>
                    </div>
                    <div class="kpi-value">{{ $acceptedCount }}</div>
                    <div class="kpi-meta">{{ $acceptanceRate }}% approval rate</div>
                    <div class="kpi-bar" @style(['--bar-width: '.$acceptanceRate.'%' => true])><div class="kpi-bar-fill"></div></div>
                </div>
                <div class="kpi-card pending">
                    <div class="kpi-card-top">
                        <span class="kpi-label">Under Review</span>
                        <span class="kpi-icon"><i class="fas fa-clock"></i></span>
                    </div>
                    <div class="kpi-value">{{ $pendingCount }}</div>
                    <div class="kpi-meta">Awaiting supervisor decision</div>
                    <div class="kpi-bar" @style(['--bar-width: '.$pendingRate.'%' => true])><div class="kpi-bar-fill"></div></div>
                </div>
            </div>

            @if ($rejectedCount > 0)
                <div class="form-pro-notice compact">
                    <i class="fas fa-exclamation-triangle"></i>
                    <div>
                        <strong>{{ $rejectedCount }} idea(s) rejected</strong>
                        <span>Review the rejection reason below and submit a revised proposal if needed.</span>
                    </div>
                </div>
            @endif

            @if ($myIdeas->isEmpty())
                <div class="empty-state">
                    <div class="empty-state-icon">
                        <i class="fas fa-lightbulb"></i>
                    </div>
                    <h3>No idea submissions yet</h3>
                    <p>You have not proposed any project ideas. Go to the dashboard to submit your first concept with your team.</p>
                    <div class="empty-state-actions">
                        <a href="{{ url('/StudentDashboard') }}" class="btn-primary">
                            <i class="fas fa-lightbulb"></i> Submit an Idea
                        </a>
                        <a href="{{ url('/StudentDashboard/acceptance') }}" class="btn-secondary">
                            <i class="fas fa-clipboard-check"></i> View Project Requests
                        </a>
                    </div>
                </div>
            @else
                <div class="acceptance-toolbar">
                    <div class="toolbar-title">
                        Idea History
                        <span>· {{ $totalCount }} record(s)</span>
                    </div>
                    <div class="filter-group" role="group" aria-label="Filter ideas">
                        <button type="button" class="filter-btn active" data-filter="all">All</button>
                        <button type="button" class="filter-btn" data-filter="accepted">Approved</button>
                        <button type="button" class="filter-btn" data-filter="pending">Under Review</button>
                        <button type="button" class="filter-btn" data-filter="rejected">Rejected</button>
                    </div>
                </div>

                <div class="request-list" id="idea-list">
                    @foreach ($myIdeas as $request)
                        @php
                            $isAccepted = $request->accepted == 1;
                            $isRejected = $request->rejected == 1;
                            $status = $isAccepted ? 'accepted' : ($isRejected ? 'rejected' : 'pending');
                            $teamMembers = $request->members->sortBy('position');
                        @endphp
                        <article
                            class="request-item {{ $isAccepted ? 'is-accepted' : ($isRejected ? 'is-rejected' : 'is-pending') }}"
                            data-status="{{ $status }}">
                            <div class="request-item-body">
                                <div class="request-item-top">
                                    <div>
                                        <div class="request-ref">
                                            <i class="fas fa-hashtag"></i> IDEA-{{ str_pad($request->id, 4, '0', STR_PAD_LEFT) }}
                                        </div>
                                        <h3>{{ $request->projectname }}</h3>
                                    </div>
                                    <span class="status-pill {{ $status }}">
                                        <i class="fas fa-{{ $isAccepted ? 'check-circle' : ($isRejected ? 'times-circle' : 'hourglass-half') }}"></i>
                                        {{ $isAccepted ? 'Approved' : ($isRejected ? 'Rejected' : 'Under Review') }}
                                    </span>
                                </div>

                                <div class="request-meta-grid">
                                    <div class="meta-block">
                                        <label>Supervisor</label>
                                        <span>{{ $request->supervisor->name ?? 'Supervisor unavailable' }}</span>
                                    </div>
                                    <div class="meta-block">
                                        <label>Team Size</label>
                                        <span>{{ $request->count }} member(s)</span>
                                    </div>
                                    <div class="meta-block">
                                        <label>Submitted On</label>
                                        <span>{{ $request->created_at?->format('M d, Y') ?? '—' }}</span>
                                    </div>
                                </div>

                                @if ($isRejected && $request->reason)
                                    <div class="form-pro-notice compact">
                                        <i class="fas fa-comment-dots"></i>
                                        <div>
                                            <strong>Rejection reason</strong>
                                            <span>{{ $request->reason }}</span>
                                        </div>
                                    </div>
                                @endif

                                <div class="meta-block team-block">
                                    <label>Team Members</label>
                                    <div class="avatar-stack">
                                        @foreach ($teamMembers as $member)
                                            <span class="avatar-chip {{ $member->user_id === $currentUserId ? 'is-you' : '' }}">
                                                <span class="avatar-initial">{{ strtoupper(substr($member->user->name, 0, 1)) }}</span>
                                                {{ $member->user->name }}{{ $member->user_id === $currentUserId ? ' (You)' : '' }}
                                            </span>
                                        @endforeach
                                    </div>
                                </div>

                                <div class="request-progress" aria-label="Idea progress">
                                    <div class="progress-step done">
                                        <div class="progress-dot"><i class="fas fa-check"></i></div>
                                        <div class="progress-label">Submitted</div>
                                    </div>
                                    <div class="progress-step {{ $isAccepted || $isRejected ? 'done' : 'active' }}">
                                        <div class="progress-dot"></div>
                                        <div class="progress-label">Review</div>
                                    </div>
                                    @if ($isRejected)
                                        <div class="progress-step rejected done active is-final">
                                            <div class="progress-dot"><i class="fas fa-times"></i></div>
                                            <div class="progress-label">Rejected</div>
                                        </div>
                                    @else
                                        <div class="progress-step {{ $isAccepted ? 'done active is-final' : '' }}">
                                            <div class="progress-dot">
                                                @if ($isAccepted)
                                                    <i class="fas fa-check"></i>
                                                @endif
                                            </div>
                                            <div class="progress-label">Approved</div>
                                        </div>
                                    @endif
                                </div>
                            </div>
                        </article>
                    @endforeach
                </div>
            @endif

            <div class="settings-note acceptance-note">
                <i class="fas fa-info-circle"></i>
                Supervisors review ideas individually. Contact them through the dashboard if you need clarification on a decision.
            </div>
        </div>

        <div class="dashboard-footer-accent"></div>
    </div>
@endsection

@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const filterButtons = document.querySelectorAll('.filter-btn');
            const ideaItems = document.querySelectorAll('.request-item');

            if (!filterButtons.length) return;

            filterButtons.forEach(function(button) {
                button.addEventListener('click', function() {
                    const filter = button.dataset.filter;

                    filterButtons.forEach(function(btn) {
                        btn.classList.toggle('active', btn === button);
                    });

                    ideaItems.forEach(function(item) {
                        const status = item.dataset.status;
                        item.classList.toggle('hidden', filter !== 'all' && status !== filter);
                    });
                });
            });
        });
    </script>
@endpush
