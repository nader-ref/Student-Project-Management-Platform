    ...
    <div id="dashboard" class="tab-content active-content">
      ...
      <div class="cards-grid">
        ...
        @foreach($requests as $request)
         @if($request->nameone == Session::get('name')||$request->nametwo == Session::get('name')||$request->namethree == Session::get('name'))
            <div class="stat-card">
            <h3><i class="fas fa-folder-open" style="color: #0a2942;"></i>Request</h3>
            <div class="value">{{$request->projectname}} <span class="unit">project Name</span></div>
              @if($request->accepted == 1)
               <div  class="bg-green-600 hover:bg-gray-800 text-white px-8 py-4 text-lg font-bold rounded-lg border-2 border-black shadow-md transition-all flex items-center gap-3">
                 accepted
               </div>
              @elseif($request->rejected == 1)
               <div  class="bg-red-600 hover:bg-gray-800 text-white px-8 py-4 text-lg font-bold rounded-lg border-2 border-black shadow-md transition-all flex items-center gap-3">
                 rejected
                 Reason:{{$request->reason}}
               </div>
              @else
               <div  class="bg-blue-600 hover:bg-gray-800 text-white px-4 py-2 text-lg font-bold rounded-lg border-2 border-black shadow-md transition-all flex items-center gap-3">
                 pending 
               </div>
              @endif
              
              <div >For more information check with your supervisors</div>
            </div>
